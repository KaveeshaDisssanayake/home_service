        ...
        {businessList && businessList.length > 0 ? (
          businessList.map((business, index) => (
            <Link href={'/details/'+business.id}
              key={index}
              className='shadow-md rounded-lg cursor-pointer hover:shadow-lg hover:shadow-primary hover:scale-105 transition-all ease-in-out'
            >
              <Image
              ...
              <div className='flex flex-col items-baseline p-3 gap-1'>
                <h2 className='p-1 bg-purple-200 text-primary rounded-full px-2 text-[12px] font-semibold'>
                  {business.category ? business.category.name : 'Category not available'}
                </h2>
